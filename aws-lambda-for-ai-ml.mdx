...
You *could* handle long-running tasks by building an intricate AWS Step Functions state machine. But that means building specialized infrastructure custom-fit to one workflow, when what you actually want is just to run code.

Early on, you don't want to do a bunch of AWS architecture work. You just want it to work. AgentCore lets a single instance execute for **hours**, which sidesteps the state-machine complexity and just lets the agent run.
