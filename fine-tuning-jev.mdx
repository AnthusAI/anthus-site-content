...
That leaves the two options everyone talks about: build a labelled dataset first and train your own classifier, or take the zero-shot answer as it comes. There's a third way, and it's the one we use. Start classifying immediately with the decision model. Review the decisions as it makes them, and label the ones you disagree with. Those labels are far cheaper than a dataset built up front, and they're what continuously aligns the classifier from then on.

So we left Jev exactly as it is and built a small system around it that learns from exactly that agree-or-disagree feedback. It learned in two steps, and the order matters.
...
Scorecards are how we've run production classifiers for years. On Plexus a scorecard is a named, versioned set of questions and a head, evaluated against held-out items and promoted only when the numbers say so, with hundreds of them live at once for one client. The YAML in this repo is the small version of that object.
...
That gate is the same shape as the review workflow we run in production: a proposed change, its numbers on held-out data by slice, a person approving the diff, and a record of who approved what. It's what lets a reviewer team steer a classifier without anyone touching weights, and it's why the audit trail matters as much as the accuracy.
...
This is the playbook we run for clients. If you're running it yourself:
...
Clone the [repo](https://github.com/AnthusAI/Jev-Flywheel) and run `make demo`: it rebuilds the recorded run offline in about ten seconds, with no keys, no network and no model. It's deliberately small.

## We run this loop for clients, at scale

The repo is the small, reproducible version of a loop we've operated in production for years. Reviewers correct the model's verdicts and say why. The explanation becomes a stated policy. The system gets more trustworthy month over month, and every change is recorded so you can see what moved and whether it was better. We've run that for a call-center QA operation across hundreds of scorecards and millions of interactions, and it's the [case study](/blog/call-criteria/) behind every number on this page about what the method does when the labels are real.

[Plexus](/platform/plexus) is where we run it: the questions, the fitted heads, the calibration, the gates, the reviewer workflow, the audit trail, and the distillation step when you're ready to own the classifier. The "what we haven't tested" list above is about this repo, not about the method. The part still to show here, continuous learning from a real review team, is the part we've been doing longest.

If you have a judgment task at volume and a team that can't review more than a sliver of it, we'll run this for you: your data, your reviewers, a fixed first month, and a scorecard you can inspect at the end of it. [Here's how an engagement works](/engage).
