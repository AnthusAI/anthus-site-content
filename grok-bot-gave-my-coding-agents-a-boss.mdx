...
tags:
  ...
  - Grok Bot
excerpt: |
  Grok Bot's organizational paradigm clicked by about the fourth day: persistent named teammates, channels as shared rooms, bot-to-bot handoffs, and work that kept going after the laptop closed. Ryan started cloning it as Chatticus — not because he wanted more Grok, but because the shape of the product was obvious and the model behind it wasn't.
...
This is completely nuts — and it is also the clearest product signal I've had in a year of coding agents. Grok Bot succeeded hard enough to make a larger category obvious. What I want next is not more Grok. It's the version of that category I started building as [Chatticus](https://chattic.us): [Shared spaces for people and bots.](https://chattic.us)
...
It is a shared, collaborative space where people and bots work together around common files, tools, and a system of authority and approvals — like an office, not a chat window. One shared filing cabinet; compute that flexes. Text-only turns can stay computerless and escalate when a tool actually needs a computer. Consequential actions — sending, publishing, buying, deleting, changing permissions — pause at an approval boundary you can inspect.

Chatticus is not Grok Bot with a different logo. It is a direct response to Grok Bot's fixed model and fixed compute boundaries.

What's live today includes durable named bot records and memory, turn-scoped progress over server-sent events, household tasks and turn journals, and development, staging, and production boundaries. Proven in development — not yet shipped as product polish — are computerless turns that escalate on first computer use, one shared computer start across concurrent turns, durable handoff and recovery across worker failure, and policy checks at consequential system sinks. Shipping next are a complete skills and routines authoring surface, approval cards and human takeover in the product app, role and reporting-line controls for named teammates, and a polished computer preview and work history.

I'm not claiming the roadmap as shipped. The ledger on [chattic.us](https://chattic.us) is explicit about what's built, what's proven, and what's next — and that's the honest pitch. The organizational paradigm Grok Bot demonstrated is real. The product I want is one where several humans and bots share that paradigm in infrastructure the customer controls, with discipline about who did what, on whose authority, and what happens when it's wrong.
...
Grok Bot succeeded hard enough to make the larger category obvious. [Chatticus](https://chattic.us) is the attempt to build the version I want — persistent named teammates, shared rooms, durable handoffs, flexible compute, human authority boundaries, and eventually several humans and bots working in one place. Grok Bot gave my coding agents a boss. What I need next is the organization itself.
