...
Change "he" to "she" in a lawyer's online bio and [Laya](https://huggingface.co/convaiinnovations/laya), the open model, reads the attorney as a paralegal on 17.8% of two thousand real bios; [Jev](https://typesafe.ai/blog/introducing-system-one-models-and-jev), the hosted model, on 3.9%. Rank the same bios by either model's confidence, keep the top 500, and women attorneys make the cut at 0.48 of the men's rate on Laya and 0.85 on Jev. [The One-Word Test](/blog/one-word-test/) is the measurement; the question here is what a deployer can do about it above the model. <mark>The learning loop that raises accuracy leaves the bias alone.</mark> A gate that rejects any question whose answer moves with the pronoun keeps out the obvious ones, and where the harm is largest it made the shortlist worse. Averaging each bio with its pronoun-swapped twin is the cheapest thing that helped. Fine-tuning, which we predicted would make things worse, didn't.

Three fixes get proposed for a shift like this: reject any question that looks gendered, average away a cue you can swap, or retrain on corrected labels and hope it doesn't relearn the pattern. We run all three as standing parts of one loop for clients: a gate a new question has to clear before it joins the scorecard, an averaging fallback for any cue you can swap, and fine-tuning as the option when the classifier itself needs to change, all checked continuously against real reviewer feedback rather than run once and left alone. Which one actually helps a given outcome isn't obvious from the name of the fix; it depends on the decision, and we measured it. The loop's machinery is in [Fine-Tuning Jev](/blog/fine-tuning-jev/).
...
This is the same promotion gate we run in production, with one more check layered on. A candidate scorecard is scored on held-out data, a person reads the diff, and now that diff carries a counterfactual number alongside accuracy. It's what lets a reviewer approve a change without also signing off on a bias nobody measured.
...
That's why we gate on the decision's own outcome in production: a scorecard change gets checked against the ranking or shortlist it actually produces, by group, before it's promoted. A question that passes every invariance test we know to write can still move the number that matters, so we measure the outcome itself every time a scorecard changes.
...
## We run this triage for clients, at scale

The arms above are the small, reproducible version of a loop we've operated in production for years. A reviewer corrects a verdict and explains why, and that explanation becomes a candidate question, an averaging rule, or a fine-tuning cycle, checked against held-out data and the decision's own outcome before anything ships. We've run that loop for a call-center QA operation across hundreds of scorecards and millions of interactions, and the [case study](/blog/call-criteria/) is what it looks like when the gate, the reviewer workflow, and the audit trail run continuously.

[Plexus](/platform/plexus) is where we run it: the scorecard promotion gate, the outcome-level check by group, and the record of who approved each change.

If you're shipping a classifier that touches people and want the gate, the averaging fallback, and the fine-tuning option checked against your own reviewers, we'll set that up. [Here's how an engagement works](/engage).
