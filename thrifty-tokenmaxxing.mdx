...
excerpt: "They offer you Fast like a free first hit that isn't free. Resist the premium default, watch the leftover clocks, and spend included quota on context hygiene — not on-demand."
preview_image: "./images/thrifty-tokenmaxxing.png"
images:
  - "./images/thrifty-tokenmaxxing.png"
state: draft
tags:
  - articles
  - ai
...
<BlogImage
  ...
  name="thrifty-tokenmaxxing.png"
  className="centered"
  alt="Thrifty tokenmaxxing"
/>
...
That's the rate trick. I wrote the tax table in [Never Use Fast](/blog/never-use-fast/). You can dodge the multiplier and still get crushed by volume. Context is the bill. Every turn re-reads the pile. Leave junk in the window and you pay for it again, and again.
...
Other people's thrifty-coding writing already says this. [Tokenminning](https://tokenminning.ai/ides) walks the same sequence: measure, route, trim context, tighten prompts. [Continuum's Claude Code list](https://continuumcode.ai/guides/reduce-claude-code-costs/) is the strongest single-tool version. The crowd already covers resist-flagship. Maximize-the-included-pool is the thinner public lane. What's below is the volume catalog.
...
Auto is a router at the routed model's [list price](https://cursor.com/docs/models-and-pricing). It is not a discount. Don't leave Auto on as a savings strategy in 2026.
...
Usage page in the dashboard. No first-party leftover CLI. [CodexBar](https://github.com/steipete/CodexBar) covers Cursor leftover.
...
Live leftover: [CodexBar](https://github.com/steipete/CodexBar). Don't treat tiny one-off bars as the community.

### Claude Code

Included weekly, then Extra Usage. `/context` and `/compact` are first-party. `/clear` between jobs. `CLAUDE.md` every turn except Explore/Plan. Fork copies parent; non-fork is brief-only. Effort / fast mode is a product flag, not "the included pool." Pin cheap models on sub-agents. Hooks that keep test failures and drop passing noise. `isolation: worktree` when you're paralleling. [ccusage](https://github.com/ccusage/ccusage) for historical logs.
...
Grok 4 Fast vs Grok 4.1. SuperGrok vs Heavy. Context window is a billed shape — don't treat max context as free headroom. On paid Cursor plans, [Grok Bot has its own weekly pool](https://cursor.com/help/grok-bot/plans); unused dies.

The interesting thrift isn't the model picker. After the 0.18.0 leak, the context layer is public: freeze memory and profile to `compactionEpoch`, append identity updates, spill MCP payloads over 12KB, keep the serialized tool surface stable. That's [why the system prompt has to stay frozen](https://yage.ai/share/grok-bot-context-engineering-en-20260827.html). Product review is still [Grok Bot Gave My Coding Agents a Boss](/blog/grok-bot-gave-my-coding-agents-a-boss/).

### Copilot, only as the Auto exception

GitHub Copilot's docs still list a [10% Auto discount](https://docs.github.com/en/copilot/tutorials/optimize-ai-usage). Cursor and Claude Auto bill at list. `/new` vs `/compact`. Session credit caps exist. Don't copy Copilot's Auto advice onto Cursor.

## Monitor

Two jobs. Don't mix them.

**[CodexBar](https://github.com/steipete/CodexBar)** (~21k GitHub stars): live leftover. Menu bar plus CLI. Codex, Claude, Cursor, Copilot, Grok, Antigravity. Reset clocks and remaining percent. This is the one that chases vendor API changes.

**[ccusage](https://github.com/ccusage/ccusage)** (~18k stars, ~85k npm/week): historical logs. What did this week cost, by session and model. Not live remaining-%.

Optional if you want both in a TUI: [tokscale](https://github.com/junhoyeo/tokscale). Skip tiny glue. Real, and not the community.
...
- [The Year Coding Became a Commodity](/blog/ai-coding-cost-collapse-2026/) — Jevons and the market.
