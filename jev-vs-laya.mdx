...
We happened to have the right test bench. It gives both models the same texts, the same questions and the same 140 rounds of human feedback, and changes one thing: which model answers. On that test Jev came out ahead, 87% accuracy to Laya's 80%, and our feedback layer helped both without closing the gap. Then we did the one thing only an open model allows: we retrained Laya's own weights on that same feedback, and it jumped to 90%, past both. We'd predicted in writing that it wouldn't.

Either engine gives you the real choice underneath: take the answer as it comes, or build a reviewer loop that continuously aligns it. We run that loop the same way regardless of which engine sits underneath it: score immediately, have a reviewer agree or disagree with the verdicts, and use those labels to refit the weights and, when the disagreements keep coming, add a question. That loop is what closes most of the gap between the two engines here, whichever one you pick, and it costs far less than building a labeled dataset before you start.

This piece is the full comparison, including what retraining Laya costs you.
...
Scorecards built this way are how we run production classifiers for clients: a versioned set of questions and a fitted head, evaluated against held-out items and promoted only when the numbers clear a bar. We've run hundreds of them live at once for one client, so the shape here, small as it is, is the same shape we use at scale.
...
That's the same job confidence gating does in production: rank verdicts by calibrated confidence and let the ones above a threshold through automatically, routing the rest to a person. A well-calibrated head is what makes that threshold mean what it says, whichever engine sits underneath it.
...
<figure className="center-full-image">
  <BlogImage
    ...
    className="full"
    alt="Bar chart of held-out accuracy with the same 140 labels. Laya head-only fine-tune 0.659, Laya alone 0.722, Jev alone 0.768, Laya with the layer 0.802, DistilBERT fine-tune 0.835, Jev with the layer 0.870, Laya full fine-tune 0.896. Black lines show the range over three seeds."
  />
  <figcaption className="image-caption">
    Seven bars, the same 600 held-out items. Light bars are the engines with no
    labels; every other system got the same 140. Solid bars have the flywheel
    layer. Hatched bars are gradient fine-tunes (mean of three seeds, with the seed range as a black
...
<figure className="center-full-image">
  ...
  />
  <figcaption className="image-caption">
    Full fine-tunes on random pool draws, three seeds per size. The solid line
    is the pre-registered curve; the dotted lead-in is an exploratory run at 20,
    40 and 80 labels. The dashed lines are the two flywheel results at 140 labels.
  </figcaption>
</figure>
...
<figure className="center-full-image">
  ...
  />
  <figcaption className="image-caption">
    We fine-tuned Laya on one question, the sentiment verdict. These are the
    eight questions we didn't train on, and how often its answer to each one
    changed. Mean of three seeds; the black line spans them. Changed doesn't
    mean wrong: there's no answer key for these.
  </figcaption>
</figure>
...
<figure className="center-full-image">
  ...
  />
  <figcaption className="image-caption">
    Rank each system's verdicts by its own confidence and accept from the top
    until accuracy would drop below 95%. Black lines span three training seeds.
    Exploratory; we didn't pre-register this comparison.
  </figcaption>
</figure>
...
The repo is deliberately small: one score, a logistic head, no database.

## We run this comparison for clients, at scale

Choosing an engine and aligning it are two different decisions, and this repo is the small, reproducible version of a loop we've run in production for years to make the second one work regardless of the first. A reviewer corrects a verdict and says why; the explanation becomes a stated policy, one more question or one more weight in the scorecard, and the system gets more trustworthy month over month with an audit trail behind every change. We've run that loop for a call-center QA operation across hundreds of scorecards and millions of interactions, and it's the [case study](/blog/call-criteria/) behind the method, whichever engine sits underneath it.

[Plexus](/platform/plexus) is where we run it: the questions, the fitted heads, the calibration, the promotion gates, and the reviewer workflow that decides whether Jev, Laya, or something else answers for you.

If you're weighing a hosted engine against open weights for a real scorecard, we'll run this comparison and the alignment loop for you, on your data. [Here's how an engagement works](/engage).
