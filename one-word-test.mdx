...
Two answers get proposed when someone asks whether a deployed classifier discriminates: benchmark it once before launch, or trust the accuracy number and move on. We run a third for clients: audit the classifier that's already live, on real records, broken out by slice, before it goes in front of people and again on a schedule after. That's the one-word test, run continuously instead of once, on a name or a pronoun, so a shift shows up as a number your team tracks instead of a story someone tells after the fact.
...
This is the same shape of check we run before a scorecard ships in production: score held-out items overall and by slice, since a model can look fine on average and still be systematically wrong for one group. Every promotion in [Plexus](/platform/plexus) gets checked that way, and a regression on a slice holds up the release even when the aggregate number improved.
...
We gate the same way in production: a proposed scorecard change is scored on held-out data broken out by group, a person reads the diff, and it ships only after they approve it. That habit is what would catch a ratio moving to 0.66 before launch instead of after.
...
The audit trail is what makes a shift like this visible. Every promoted scorecard in Plexus records who approved which version and why, so when a verdict moves on a slice, you can trace it to a specific change and a specific signoff rather than discovering it as a lawsuit or a public post.
...
The checks here are the small, reproducible version of a loop we run continuously in production. When a reviewer disagrees with a verdict and says why, that explanation becomes a question the scorecard asks going forward, so the same mistake doesn't recur unnoticed. We've run that loop for a call-center QA operation across hundreds of scorecards and millions of interactions; the [case study](/blog/call-criteria/) is what continuous review looks like at that scale.

[Plexus](/platform/plexus) is where we run the per-slice checks, the approval gate, and the audit trail that catch a shift like the ones measured here before it ships.

If you have a classifier making decisions about people and no standing check for verdict shifts by name or pronoun, we'll build one against your own data. [Here's how an engagement works](/engage).
