...
The large language models (LLMs) behind generative AI are also predictors: They work by predicting the 'next' word in a sequence.  The sequence starts with your prompt: "Knock, knock..."

In this case it's easy to figure out what word is likely to come next.  If someone says, "Knock, knock", what's the next thing you're supposed to say?  "Who's there?"
...
LLMs lower the barrier to entry for automating decisions. Cloud-based APIs let organizations tap into powerful, pre-trained models without managing complex infrastructure. These models come with broad knowledge that can be applied to specific tasks, even when domain-specific data is scarce. And you can rapidly improve LLM-based classifiers through prompt engineering and fine-tuning, often with far fewer examples than traditional ML models require.

An AI API can serve as a flexible platform for easily and cheaply creating custom classifiers, whatever your situation:

#### No training data
You can get started even if you don't have any training data at all.  You can use the model's generalized pre-training to make decisions.  You can get up and running quickly with basic prompt engineering and then you can align your solution over time by looking at how well it performs.
...
#### Lots of examples
If you can create thousands of examples of 'correct' decisions then you can use machine-learning techniques for evaluating the performance of your solutions.  You can gain the additional benefit of accurate confidence values to tell you how much you can trust each individual decision.
