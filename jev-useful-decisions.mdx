---
title: "Making Decisions Instead of Generating Text"
slug: "making-decisions-instead-of-generating-text"
redirect_from:
  - "jev-useful-decisions"
date: "2026-09-17"
authors:
...
Anthus has spent the last several years building post-call quality analysis at industrial scale. After a customer-service conversation ends, our systems analyze the recording or transcript against a scorecard: did the representative resolve the request, follow policy, or leave something unanswered? We began with conventional ML models, then increasingly used LLMs when the work called for more nuanced language understanding. LLMs brought a mismatch: they produce open-ended text, and a QA scorecard needs a bounded decision with a usable measure of uncertainty.

Jev points at a different way to close that gap. Instead of fitting a text generator into a classifier's job, TypeSafe designed Jev to return typed decisions and probabilities directly: give it a state and a set of bounded questions, and each one comes back as a choice, a score, or a probability, no parsing or retries required to get there. For post-call QA, that could move the classifier's home from a wrapper we build around a text generator to a model whose native output already is the classifier, with the application still responsible for scorecards, thresholds, and review policy.
...
That's the bet TypeSafe is making with Jev's design, and it's a bet our own production work is positioned to test: we've been running exactly that shape of decision for years, just without a model built to return it natively.

Our [Plexus MLOps platform](https://plexus.anth.us/) carries that work into production scorecards. Teams define criteria as scores, evaluate them against labeled examples, then feed expert corrections into a [Feedback Alignment Optimizer](https://plexus.anth.us/documentation/procedures/optimizer) that proposes and tests score changes against both recent feedback and a fixed regression set. It is a practical classifier data flywheel: each measured round gives the next one better evidence to work from. Each scorecard is a versioned set of questions and a small fitted head that turns the answers into a verdict, evaluated against held-out labels and promoted only when it beats the version it's replacing. We run hundreds of them live at once for one client, and every promotion is on record: what changed, on what evidence, and who approved it.
...
We described an earlier version of that approach in [Fine-Tuned Classification with Confidence](/blog/fine-tuned-classification-with-confidence/). The details have evolved, but the operational lesson has held: classification quality lives in the whole system. The prompt, model, rubric, parser, threshold, evaluation set, and human review process all matter.
...
Jev's [interface](https://docs.typesafe.ai/introduction) accepts shared state and a collection of questions. It exposes three decision shapes:
...
That's also how we get a classifier onto a scorecard now, on Jev or off it. The industry usually offers two ways to get there: build a labelled dataset first and train your own classifier, or take the zero-shot answer as it comes. We use a third way. Start classifying immediately with the decision model, review what it decides as it decides it, and label the cases a reviewer disagrees with. Those labels are far cheaper than a dataset built up front, and they're what continuously aligns the classifier from then on, on every scorecard we run in production.
...
In production we set that threshold per question: routine verdicts above the bar auto-accept, the middle band routes to a reviewer, and a compliance question keeps a stricter bar than a missed greeting. That gate is what makes a probability actionable instead of decorative, and it's the same policy question Jev's confidence field raises here.

This is also where human feedback keeps its central role. Experts define the rubric, adjudicate disputed examples, examine failure modes, and improve the evaluation set. In Plexus, [human-in-the-loop review](https://plexus.anth.us/solutions/optimizer-agents) turns those corrections into feedback for the optimizer. Their decisions create the data flywheel that makes the whole system more useful over time. Jev could simplify the model-facing interface inside that loop; it doesn't replace the loop.
...
## We run this for clients, at scale

That bounded comparison is the small version of a loop we already operate in production. Reviewers correct a scorecard's verdicts, an optimizer treats their corrections as data, and every promoted change is measured against a fixed regression set before it goes live. We've run that loop for a call-center QA operation across hundreds of scorecards and millions of interactions, documented in the [case study](/blog/call-criteria/).

[Plexus](/platform/plexus) is where we run it: the questions, the fitted heads, the calibration, the promotion gates, and the audit trail. Testing Jev against a scorecard we understand is exactly the kind of engagement we run for clients: your data, your reviewers, a fixed comparison, and a scorecard you can inspect at the end of it. [Here's how an engagement works](/engage).
